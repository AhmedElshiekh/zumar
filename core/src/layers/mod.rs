...
impl ZumarBlock {
    ...
    /// Forward عادي (للتدريب - يحتاج كل الذاكرة)
    ...
    /// Forward مع Activation Checkpointing + Sparse MoE (للاستدلال)
    ...
    /// بناء ZumarBlock من packed blocks مع Sparse MoE
    pub fn from_packed_blocks(
        ...
        let packed_experts: Vec<PackedBlockRef> = layer_blocks[5..5+num_experts].to_vec();
...
impl ZumarModel {
    ...
    pub fn forward(&mut self, x: &Tensor) -> Result<Tensor> {
        ...
        for layer in &mut self.layers {
            hidden_states = layer.forward_checkpointed(&hidden_states)?;
        }
        hidden_states = self.final_norm.forward(&hidden_states)?;
        self.lm_head.forward(&hidden_states)
    }

    /// بناء النموذج من packed blocks مع Sparse MoE + 1-bit Embedding
    pub fn from_packed_blocks(
        vocab_size: usize, in_dim: usize, num_layers: usize,
        ...
        for i in 0..num_layers {
            ...
            if end <= blocks.len() {
                let layer_blocks = &blocks[start..end];
...
/// فك ضغط Embedding من 2-bit إلى FP16 (نصف ذاكرة FP32)
